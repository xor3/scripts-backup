...
//{{%PRESERVE%
// Put your codes here
//plugins.options["K2Emacs.editor"]    = "D:\\Program Files\\emacs\\bin\\emacsclientw.exe";
...
ext.add("previous-page", function () {
	...
	text = links[i].text;
	if(text.match("上一") || text.match("前一")
	   || (text.match(/prev/i))
	   || (text == '<') || (text == '<<')) {
     ...
     // 以下通过class来找上一页
    var nexts = document.getElementsByClassName("pre");
    for(i = 0; i < nexts.length; i++) {
	links = nexts[i].getElementsByTagName("a");
	for( j = 0; j < links.length; j++) {
	    document.location =  links[j].href;
	    return;
	}
    }
}, "Previous page");
...
ext.add("next-page", function () {
    ...
    for(i = 0; i < links.length; i++) {
        ...
        //        || (links[i].text == '下页')   || (links[i].text == '下一节')
        //        || (links[i].text == 'Next')    || (links[i].text == 'next')
        //        || (links[i].text == '>')       || (links[i].text == '>>'))
        //     document.location = links[i].href;
	text = links[i].text;
	if(text.match("下一") || text.match("后一")
	   || (text.match(/next/i)) || links[i].title.match("下一")
	   || (text == '>') || (text == '>>')
	   || (text == "下页")) {
...
//               '[',              
//               function (ev) {
//                   ext.exec("previous-page");
//               }, 'previous page');

key.setViewKey(["C-c", "d"],
    function (ev, arg) {
        ext.exec("baidu_translation", arg);
    }, ext.description("baidu_translation"), true);

key.setViewKey('s',function(ev){ext.exec("history-show");},'show history');
key.setViewKey('v', function (ev, arg) {
    ext.exec("bmany-list-all-bookmarks", arg, ev);
}, 'bmany - List all bookmarks');

key.setGlobalKey(['M-k'], function (ev) {
